...
[![npm version](https://badge.fury.io/js/lambda-ses-forwarder.svg)](https://www.npmjs.com/package/aws-lambda-ses-forwarder)
[![Travis CI test status](https://travis-ci.org/singularity-is/lambda-ses-forwarder.svg?branch=master)](https://travis-ci.org/singularity-is/aws-lambda-ses-forwarder)
[![Test coverage status](https://coveralls.io/repos/github/singularity-is/aws-lambda-ses-forwarder/badge.svg?branch=master)](https://coveralls.io/github/singularity-is/aws-lambda-ses-forwarder?branch=master)
...
redirects, or setup amazon Workmail, use SES to receive email, 
and the included Lambda script to process it and send it on to the chosen destination.

This is enhanced version of `aws-lambda-ses-forwarder` forked from [Arithmetric](https://github.com/arithmetric/aws-lambda-ses-forwarder)
It removes all email files from S3, that are not mapped in configuration, and
moves all successfully forwarded emails to separate folder.
This may be handy for users with multiple domains attached to single SES inbound rule.
Now you may setup multiple S3 buckets fetching all emails, from different verified domains,
and keep only emails related to each mapping configuration per S3 bucket.
...
2. In AWS Lambda, add a new function and skip selecting a blueprint.
 ...
 Runtime is set to Node.js 12.x. (Node.js 10.x and Node.js 14.x are also supported.)
...
See [example](https://github.com/singularity-is/aws-lambda-ses-forwarder/tree/master/example)
